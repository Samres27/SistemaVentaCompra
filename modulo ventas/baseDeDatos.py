...
def crearBaseDeDatos():
    ...
    try:
        ...
        conectar.execute("""CREATE TABLE IF NOT EXISTS articulo(id integer primary key autoincrement,tipo text,nombre text,valor float)
                         """) 
        #crear venta id,cliente,fecha,nroBoleta,producto,precio,pesoBruto,pesoNeto,total
        ...
        lsPollo=["Pollo","P con Menud","P sin Menud","S con Menud","S sin Menud","P. Especial","PiernaMus",
...
def obtenerDatosTipo(tabla,tipo):
    conectar=sqlite3.connect("./silohDataBase.db")
    try:
        #obtener los valores del tipo
        vl=conectar.execute("""select id,tipo,nombre,valor from '%s' where tipo='%s'
                         """%(tabla,tipo)) 
        valores=[]
        for x in vl:
            valores.append(x)
...
def devolverListaDeItemPrecio(listas):
    ...
    respuestat=[]
    for x in listas:
        respuestap+=obtenerDatosTipo(tabla=tabl,tipo=x)
